"""
experiments/v0.5.0/run_sequential_matrix.py — Fully sequential 7x7 matrix

Runs each game one at a time in a single process, processing one AI-pair at a time.
Outputs pair results as JSON files, then computes Elo and report.

Fully sequential is slow (~1 game/sec) but reliable on Windows.
Total: 49 pairs x 400 games = 19,600 games ~ 5-6 hours.
"""
...
# Reuse imports at module level
from prototype.game import init_game, step_game
from prototype.eval import load_ai


def _run_seeded(seed, ai_a_name, ai_b_name, size, gen, max_turns):
...
def run_pair_seq(ai_a, ai_b):
    """Run all seeds for a single AI pair sequentially."""
    results = []
    last_pct = -1
    for g in range(GAMES):
        seed = SEED + g * 1000 + hash((ai_a, ai_b)) % 100000
        r = _run_seeded(seed, ai_a, ai_b, SIZE, "balanced", MAX_TURNS)
        results.append(r)
...
def main():
    ...
    print("=" * 70)
    print("  MINICIV v0.5.0 — SEQUENTIAL 7x7 PAIRED MATRIX")
    ...
    for idx, (ai_a, ai_b) in enumerate(all_pairs, 1):
        ...
        _save_summary_and_elo(pairs_data, elapsed=t_start)

    elapsed = time.perf_counter() - t_start
    print(f"\n  All {total} pairs completed in {elapsed:.0f}s")
    _save_summary_and_elo(pairs_data, elapsed=elapsed)
    _save_report(pairs_data)
    print("\n  ALL DONE")


def _save_summary_and_elo(pairs_data, elapsed=0):
    """Save summary.json and elo_rankings.json from current data."""
    ...
    print(f"\n  Interim Elo:\n  {'Rank':>4s} {'AI':>12s} {'Elo':>7s}")
    for rank, (name, e) in enumerate(ranked, 1):
        print(f"  {rank:>3d}. {name:>12s} {e['elo']:>6.1f}")
    print()
...
def _save_report(pairs_data):
    ...
    for row_name in ai_names:
        ...
        for col_name in ai_names:
            ...
            pct = wr.get(key, 0.5) * 100
            the_ci = ci_lookup.get(key, 0) * 100
    ...
    L.append(f"### Overall spread: {top_elo - bottom_elo:.0f} Elo points ({bottom_elo:.0f} to {top_elo:.0f})")
    L.append("")
    tiers = {"S": [], "A": [], "B": [], "C": []}
    ...
    for name in ai_names:
        for p in pairs:
            if p["ai_a"] == name and p["ai_b"] == name:
                ...
                L.append(f"- **{name}**: P0 = {p['p0_winrate']*100:.1f}% (+-{p['p0_ci95']*100:.1f}%), advantage = {adv:+.1f}%")
                break
    L.append("")
