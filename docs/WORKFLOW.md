...
### 算力调度纪律（铁律，2026-07-11 S2 立）

> **默认拉满算力(在确定性约束下)。任何中大规模 训练/评估/搜索 开跑前,先审当前调度是不是最优,不许随手单线程糊上去。**

**为什么立这条**：S2 发现整仓库纯单线程(无 rayon),32 核只用 1 核 ≈ 3%,FlatMC 矩阵跑几分钟。
搜索型 AI(FlatMC/MCTS)+ 反复"加机制→体检"迭代,算力就是迭代速度,浪费 30 倍算力=浪费 30 倍生命。

**开跑前必答的审核清单(中大规模 = 预计 >30s 或 >数百局)**：
1. **这批任务可并行吗?** paired seeds / 矩阵 pairs / 参数网格 之间独立 → 几乎都可并行。
2. **现在用了几个核?** 不确定就 `nproc` 看总核、跑时看 CPU 占用。占用 ≈ `1/核数` = 没并行。
3. **并行会破坏确定性吗?** 必须逐位可复现:每局 RNG 从自身 seed 派生(现有约定);
   聚合(reduce)**顺序无关**或"先 collect 再确定性合并"。单局内部保持串行(step 确定性)。
4. **不达标就先修调度再跑**,别用"反正能跑完"当借口——慢矩阵会拖垮整个深化循环。

**工具**：Rust 用 `rayon` 把外层独立循环换 `par_iter`(map→collect→确定性 reduce)。
并行版必须有一个测试断言"并行结果 == 串行结果"。多进程一般不必(数据并行+共享只读配置,rayon 足够)。
...
### 规划执行纪律（v0.6.2 教训，2026-07-05）

> **规则：规划落定后，主 Agent 必须按规划执行。禁止"刚讨论完分 Agent 方案，转头自己单人全做完"。**
>
> 如果规划里写了分子 Agent → 就分子 Agent。如果规划里写了 Worktree 隔离 → 就用 Worktree。
> 即使主 Agent 自己写会更快——因为规划不只是"怎么干活"，也是"怎么验证开发体系本身"。
>
> **违反此规则的情形（已发生）**：
> - 讨论完 9 Agent 并行方案后，主 Agent 自己写了 Greedy v6 工人经济——本该分给 3 个子 Agent 探索三种实现
> - 原因是"默认自己写了——因为快、因为确定不出错"
> - **正确做法**：基础设施（如 damage_dealt/taken）主 Agent 做；设计空间探索（如"经济优先 vs 数量优先 vs 安全优先"）分子 Agent 并行
>
> **判定标准**：如果一个任务有 ≥2 种合理实现方式且无法预先知道哪种更好 → 分子 Agent 并行探索。
> 如果只有一个正确答案（如 bug 修复、数据字段添加）→ 主 Agent 自己做。
