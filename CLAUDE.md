...
> ⚠️ **硬伤(2026-07-10 用户看回放发现)**: B1 攻城只在移入触发一次→**已修渐进攻城**。
> 排查又发现 **B2 move_speed / B3 range_dist 完全未实现**(骑兵移速2/弓箭手射程2/侦察兵机动 全失效),
> 兵种系统性残缺。**之前所有军事/兵种数据作废,需修完 B2/B3 重测。** → `docs/BUGS.md`
> **M1-M2 阶段归档 → `docs/planning/2026-07-10-M1M2-archive.md`**
> **下一步:修 B2/B3(实现兵种优势=深化军事核心) → 重测甜点带 → M3-B/深化内容。**
