"""
experiments/v0.5.0/run_fixed_matrix.py — 7x7 matrix with maxtasksperchild

Uses multiprocessing.Pool with maxtasksperchild=40 to avoid Windows handle leaks.
Processes one AI pair at a time, one pair per run (for reliability).

Usage (run for a single pair or all remaining pairs):
  cd D:/Dev/miniciv
  python experiments/v0.5.0/run_fixed_matrix.py
"""
...
SEED = 42
WORKERS = 6
MAX_TASKS_PER_CHILD = 40  # restart workers before handle leak


def _worker_run(args):
    """Run a single paired evaluation (2 games per seed)."""
    ...
    return {
        "seed": seed, "ai_a": ai_a_name, "ai_b": ai_b_name,
        "game1": g1, "game2": g2,
...
def run_pair_fixed(ai_a, ai_b):
    """Run all seeds for a single AI pair using Pool with maxtasksperchild."""
    ...
    for batch_start in range(0, len(tasks), BATCH_SIZE):
        ...
        with Pool(processes=WORKERS, maxtasksperchild=MAX_TASKS_PER_CHILD) as pool:
            for r in pool.imap_unordered(_worker_run, batch):
                results.append(r)
        print(f"    {len(results)}/{len(tasks)}", end=" ", flush=True)
    print()
    ...
    all_turns = [t for r in results for g in [r["game1"], r["game2"]] for t in [g["turns"]]]
    ...
    return {
        ...
        "p0_ci95": round(_ci95(p0_wr, total_games), 4),
        "ai_a_ci95": round(_ci95(ai_a_wr, total_games), 4),
        "ai_a_std": round(ai_a_std, 4),
        "conquest_rate": round(conquests / total_games, 4),
        "construction_rate": round(constructions / total_games, 4),
        "tiebreak_rate": round(tiebreaks / total_games, 4),
...
def _compute_elo(pairs, ai_names, start_elo=1500, K=32, iterations=200):
    ...
    for _ in range(n_boot):
        ...
        for _ in range(len(pairs)):
            ...
            for _ in range(iterations // 2):
                ...
                noise_a = random.gauss(0, math.sqrt(ea * (1 - ea) / max(1, n_games)))
                b_elo[ai_a] += K * (a_wr - ea + noise_a * 0.5)
                b_elo[ai_b] += K * ((1 - a_wr) - (1 - ea) - noise_a * 0.5)
    ...
    for name in ai_names:
        ...
        ci[name] = {
            ...
            "ci_low": round(bvals[max(1, len(bvals) // 40)], 1),
            "ci_high": round(bvals[min(len(bvals)-1, len(bvals) * 39 // 40)], 1),
        }
    return ci
...
def main():
    ...
    print("=" * 70)
    print("  MINICIV v0.5.0 — FIXED 7x7 MATRIX")
    print(f"  Workers: {WORKERS}, maxtasksperchild={MAX_TASKS_PER_CHILD}")
    print(f"  Batch: 40 seeds/pool")
    print("=" * 70)
    print()
    ...
    for idx, (ai_a, ai_b) in enumerate(all_pairs, 1):
        ...
        if os.path.exists(out_path) and os.path.getsize(out_path) > 400:
            ...
            print(f"  [{idx}/{total}] {ai_a} vs {ai_b} — loaded ({wr:.1f}%)")
            continue

        t0 = time.perf_counter()
        print(f"  [{idx}/{total}] {ai_a} vs {ai_b} — {GAMES*2} games...", flush=True)
        data = run_pair_fixed(ai_a, ai_b)
        ...
        pairs_data.append(data)
        wr = data["ai_a_winrate"] * 100
        print(f"    done ({elapsed:.0f}s, A_win={wr:.1f}%)", flush=True)

        # Save intermediate
        _save_interim(pairs_data, time.perf_counter() - t_start)

    elapsed = time.perf_counter() - t_start
    _save_interim(pairs_data, elapsed)
    _save_report(pairs_data)
    print(f"\n  Total: {elapsed:.0f}s")
    print("  ALL DONE")


def _save_interim(pairs_data, elapsed):
    if not pairs_data:
        return
    ...
    # Only compute Elo if we have enough pairs to be meaningful
    ai_names = sorted(set(p["ai_a"] for p in pairs_data))
    if len(ai_names) >= 2:
        elo = _compute_elo(pairs_data, ai_names)
        with open(os.path.join(OUTPUT_DIR, "elo_rankings.json"), "w") as f:
            json.dump(elo, f, indent=2)


def _save_report(pairs_data):
    with open(os.path.join(OUTPUT_DIR, "summary.json")) as f:
        summary = json.load(f)
    with open(os.path.join(OUTPUT_DIR, "elo_rankings.json")) as f:
        elo = json.load(f)
    pairs = summary["pairs"]
    ai_names = sorted(set(p["ai_a"] for p in pairs))
    ...
    L.append("")
    wr = { (p["ai_a"], p["ai_b"]): p["ai_a_winrate"] for p in pairs }
    ci_l = { (p["ai_a"], p["ai_b"]): p["ai_a_ci95"] for p in pairs }
    header = "| {:>8s} |".format("AI")
    for n in ai_names:
        header += f" {n:>12s} |"
    L.append(header)
    ...
    for rn in ai_names:
        row = f"| **{rn:>6s}** |"
        for cn in ai_names:
            pct = wr.get((rn, cn), 0.5) * 100
            ci = ci_l.get((rn, cn), 0) * 100
            row += f" {pct:>5.1f}%+-{ci:>3.1f}% |"
        L.append(row)
    L.append("")
    ...
    v4_path = os.path.join(_PROJECT_ROOT, "eval_final", "summary.json")
    ...
    top, bottom = graded[0][1]["elo"], graded[-1][1]["elo"]
    spread = top - bottom
    L.append(f"### Spread: {spread:.0f} Elo ({bottom:.0f}-{top:.0f})")
    L.append("")
    tiers = {"S":[], "A":[], "B":[], "C":[]}
    for n, e in graded:
        pct = (e["elo"] - bottom) / max(1, spread)
        if pct >= 0.8: tiers["S"].append(n)
        elif pct >= 0.5: tiers["A"].append(n)
        elif pct >= 0.2: tiers["B"].append(n)
        else: tiers["C"].append(n)
    for t in ["S","A","B","C"]:
        if tiers[t]: L.append(f"- **Tier {t}**: {', '.join(tiers[t])}")
    L.append("")
    ...
    if rule_top: L.append(f"- Best rule-based: {rule_top[0]} ({rule_top[1]['elo']:.1f})")
    if ml_top: L.append(f"- Best ML-based: {ml_top[0]} ({ml_top[1]['elo']:.1f})")
    if rule_top and ml_top:
        L.append(f"- Gap: {rule_top[1]['elo']-ml_top[1]['elo']:.0f} Elo")
    L.append("")
    L.append("### Mirror Match P0 Advantage")
    L.append("")
    for name in ai_names:
        for p in pairs:
            if p["ai_a"] == name and p["ai_b"] == name:
                adv = (p["p0_winrate"] - 0.5) * 100
                L.append(f"- {name}: P0={p['p0_winrate']*100:.1f}% +-{p['p0_ci95']*100:.1f}%, adv={adv:+.1f}%")
                break
    L.append("")
    L.append("### Victory Types")
