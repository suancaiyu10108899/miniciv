...
def _strategic_assess(gs, pid) -> dict:
    ...
    my_frontline = [u for u in my_combat
                    if _hex_dist(u.x, u.y, opp_city.x, opp_city.y, size) <= 6]
    my_city = gs.cities[pid]
    opp_frontline = [u for u in opp_combat
                     if _hex_dist(u.x, u.y, my_city.x, my_city.y, size) <= 6]

    # Enemy density around their city (for weak point detection)
    opp_near_their_city = [u for u in opp_combat
                           if _hex_dist(u.x, u.y, opp_city.x, opp_city.y, size) <= 4]
    ...
    for dx, dy in [(3, 0), (-3, 0), (0, 3), (0, -3), (2, 2), (-2, 2), (2, -2), (-2, -2)]:
        ...
        nearby = sum(1 for u in opp_combat if _hex_dist(u.x, u.y, wx, wy, size) <= 3)
    ...
    # Is city threatened?
    city_threatened = any(
        _hex_dist(eu.x, eu.y, my_city.x, my_city.y, size) <= 2
...
def _compute_force_coordination(gs, pid, assessment):
    ...
    wave_units = [u for u in my_combat
                  if _hex_dist(u.x, u.y, rally_point[0], rally_point[1], size) <= 4]
...
def _greedy_combat_v4(u, ui, gs, pid, opp_city, strategy, assessment, rally_point, wave_ready, rng):
    ...
    if hp_pct < 0.3:
        ...
        for eu in gs.units:
            if eu.alive and eu.player_id != pid:
                d = _hex_dist(eu.x, eu.y, u.x, u.y, size)
                if d <= 2:
                    near_enemy = True
                    break
        if near_enemy:
    ...
    for eu in gs.units:
        if eu.alive and eu.player_id != pid:
            d = _hex_dist(eu.x, eu.y, my_city.x, my_city.y, size)
            if d <= 2:
    ...
    if strategy == "defensive" or strategy == "defensive_construction":
        ...
        dist_to_my_city = _hex_dist(u.x, u.y, my_city.x, my_city.y, size)
    ...
    rpx, rpy = rally_point
    dist_to_rally = _hex_dist(u.x, u.y, rpx, rpy, size)
    if dist_to_rally <= 2:
...
def _do_production(gs, pid, strategy, production_counter, assessment, actions):
    ...
    enemy_near = any(
        ...
        and _hex_dist(eu.x, eu.y, my_city.x, my_city.y, size) <= 3
        for eu in gs.units
    )
