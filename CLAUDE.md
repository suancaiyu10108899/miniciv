...
> B3 range_dist射程2/B5冲锋 全修; B4遇林停本已实现; B6 Greedy非确定只影响Greedy(待修)。
> **兵种机制现在完整**(之前所有军事/兵种数据建立在残缺兵种上,已作废重测)。→ `docs/BUGS.md`
> **修完重测甜点带**: 军事显著变强, 征服/建设均衡(44%/42%), 但 Rusher(步兵)偏强(平均74%)、
> 骑兵弱(地形制约禁山遇林+HP80)。甜点带大致维持成本×2-3, 但需重新平衡(军事偏强、骑兵niche)。
> **深度增加前的探索完成**: 引擎机制扎实。下一步 = 深化游戏内容(科技强互斥→迷雾→多城),
> 或先微调军事平衡(骑兵/地形)。用 bin/table/eval/depth 体检验证每步。
