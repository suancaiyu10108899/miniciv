...
**S2「立裁判」完成:默认配置落地为甜点 + FlatMC 非循环裁判裁决"深度温和偏浅(H0)"。91 tests (+3 ignored)。**
...
> **Step 0 ✅ 完成**: rayon 并行 eval/judge(`run_pair_par`/`run_matrix_par`)+ "并行==串行"确定性测试。
> `bin/eval`/`bin/judge` 默认走并行。**性能债已还**。测试 80→91(+10 game.rs胜利路径/机制回归 +1 并行确定性)。
> **当前 Step 1 待开工**: 2→N 玩家引擎重构 + 队伍归属 + 支持度维度 + 抽象扩张。
